...
-- This SQL query calculates the average price for each product by dividing the total sales amount by the total units sold,
...
LEFT JOIN 
    UnitsSold u ON p.product_id = u.product_id -- Left join the UnitsSold table based on the product_id
                AND p.start_date <= u.purchase_date -- Ensure the purchase date falls within the price validity period
                AND p.end_date >= u.purchase_date
GROUP BY 
    p.product_id; -- Group the result by product_id to calculate the average price for each product
